...
static uint8_t __not_in_flash_func(sd_crc7)(const uint8_t *data, int size, uint8_t crc) {
    int tbl_idx;
...
static uint8_t __not_in_flash_func(spi_read_byte)() {
    uint8_t received = SPI_FILL_CHAR;
	spi_read_blocking(spi1, SPI_FILL_CHAR, &received, 1);
    return received;
}

static void __not_in_flash_func(spi_write_byte)(const uint8_t value) {
	spi_write_blocking(spi1, &value, 1);
}

static int __not_in_flash_func(sd_send_cmd)(uint8_t cmd, uint32_t arg) {
    uint8_t rv;
    int i = 0;
    uint8_t pkt[6];
...
static int __not_in_flash_func(acmd41_loop)(uint32_t arg) {
    int i = 0, rv;
...
static int __not_in_flash_func(cmd1_loop)(void) {
    int i = 0, rv;
...
int __not_in_flash_func(sd_init)() {
    int i;
    uint8_t buf[4];
...
static int __not_in_flash_func(wait_nbsy)() {
	uint8_t byte;
	size_t i = 0;
...
static void __not_in_flash_func(read_data)(size_t bytes, uint8_t *buf) {
    size_t i = 0;
	size_t tx_remaining, rx_remaining;
	
	tx_remaining = rx_remaining = bytes;
    
    while (rx_remaining || tx_remaining) {
        if (tx_remaining && spi_is_writable(spi1) && rx_remaining < tx_remaining + 8) {
            spi_get_hw(spi1)->dr = (uint32_t) SPI_FILL_CHAR;
            --tx_remaining;
        }
        
        if (rx_remaining && spi_is_readable(spi1)) {
			if(i > 511) {
...
static void __not_in_flash_func(read_scrambled)(size_t bytes, uint32_t *buf, const uint16_t *sc, uint8_t dt) {
    size_t i = 0;
	size_t tx_remaining, rx_remaining;
	uint16_t tmp;
	uint8_t *tmpb = (uint8_t *) &tmp;
	tx_remaining = rx_remaining = bytes;
    ...
    while (rx_remaining || tx_remaining) {
        ...
        if (rx_remaining && spi_is_readable(spi1)) {
			if(i > 511) {
				...
				tmpb[i & 1] = (uint8_t) spi_get_hw(spi1)->dr;
...
int __not_in_flash_func(sd_read_blocks)(uint32_t block, size_t count, uint8_t *buf, const uint16_t *sc, uint8_t dt) {
    int rv = 0;
    ...
    if(count == 1) {
        ...
        if (!sc) {
			...
			read_scrambled(514, (uint32_t *) buf, sc, dt);
		}
    }
    else {
...
DSTATUS __not_in_flash_func(disk_initialize)() {
	if(sd_init()) {
		return RES_NOTRDY;
	}
...
DRESULT __not_in_flash_func(disk_read)( BYTE *buff,     /* Data buffer to store read data */
										LBA_t sector,   /* Start sector in LBA */
										UINT  count,    /* Number of sectors to read */
										const WORD* sc, /* Buffer with scramble table */
										BYTE  dt	    /* Data type */
)
{
    if(sd_read_blocks(sector, count, buff, sc, dt)) {
		return RES_ERROR;
	}
...
DRESULT disk_write(const BYTE *buff, /* Data to be written */
                   LBA_t sector,     /* Start sector in LBA */
...
DRESULT __not_in_flash_func(disk_ioctl)(BYTE cmd,  /* Control code */
